...
myclasses = ['MW 1:00 p.m.', 'MW 2:30 p.m.', 'TR 11:00 a.m.', 'TR 12:30 p.m.', 'TR 2:00 p.m.']
...
for row in finals_rows:
    ...
    if final:
        ...
        if myclass in myclasses:
            print(f"For class: {myclass} the final is scheduled for {final[2].text} at {final[2].text}")